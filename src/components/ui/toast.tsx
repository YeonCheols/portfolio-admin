...
export function Toast({ type, message }: ToastProps) {
    ...
    switch (type) {
      case 'success':
        return {
          ...
          message: message || 'Item moved successfully.',
        };
      case 'error':
        return {
          id: 'toast-danger',
      ...
      case 'warning':
        return {
          ...
          message: message || 'Improve password difficulty.',
        };
    }
  };
  ...
  return createPortal(
    <div
      ...
      role="alert"
    >
      <div className={`inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg ${toastContent.iconClass}`}>
        {toastContent.icon}
        <span className="sr-only">{type} icon</span>
      </div>
      <div className="ms-3 text-sm font-normal">{toastContent.message}</div>
      <button
        type="button"
        className="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700"
        data-dismiss-target={`#${toastContent.id}`}
        aria-label="Close"
      >
